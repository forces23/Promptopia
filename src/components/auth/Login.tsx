...
const Login = ({ isAuthOpen, setIsAuthOpen }: AuthModalProps) => {
    const { session, setIsAuthenticated } = useAuth();
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        if (response.nextStep.signInStep && response.nextStep.signInStep === 'DONE') {
    ...
    return (
        <div>
            ...
            <div
                ...
                className={`${isAuthOpen ? 'flex' : 'hidden'} overflow-y-auto overflow-x-auto fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-full bg-black/75`}
            >
